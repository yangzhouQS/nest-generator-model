...
export class AutoBuilder {
  ...
  private processTables(tableResult: any[]) {
    ...
    // 数据表 tableName和tableSchema统一格式化
    let tables = _.map(tableResult, (t) => {
      return {
        table_name: t.table_name || t.tableName || t.name || String(t),
        table_schema:
          t.table_schema || t.tableSchema || t.schema || this.schema || null,
      } as Table;
    });
  ...
  private async mapTable(table: Table) {
    try {
      ...
      if (
        ...
        (_.some(fields, { type: 'ARRAY' }) ||
          _.some(fields, { type: 'USER-DEFINED' }))
      ) {
        ...
        const elementTypes = await this.executeQuery<ColumnElementType>(
          stquery,
        );
        ...
        elementTypes.forEach((et) => {
          ...
          if (fld.type === 'ARRAY') {
            ...
            if (
              ...
              fld.special = (
                this.queryInterface as any
              ).queryGenerator.fromArray(et.enum_values);
            }
      ...
      if (
        ...
        (_.some(fields, { type: 'DECIMAL' }) ||
          _.some(fields, { type: 'NUMERIC' }))
      ) {
