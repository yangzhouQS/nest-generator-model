...
export class AutoRelater {
  ...
  private addRelation(
    ...
    const isOne =
      (spec.isPrimaryKey &&
        !_.some(
          fkFields,
          (f) => f.isPrimaryKey && f.source_column !== fkFieldName,
        )) ||
      (!!spec.isUnique &&
        !_.some(
          fkFields,
          (f) =>
            f.isUnique === spec.isUnique && f.source_column !== fkFieldName,
        ));
    ...
    this.relations.push({
      ...
      parentProp: alias,
      parentTable: qNameJoin(
        spec.foreignSources.target_schema || schema,
        spec.foreignSources.target_table,
      ),
      childModel: modelName,
      childProp: isOne ? singularize(childAlias) : pluralize(childAlias),
      childTable: qNameJoin(
        spec.foreignSources.source_schema || schema,
    ...
    if (spec.isPrimaryKey) {
      ...
      const otherKeys = _.filter(
        fkFields,
        (k) =>
          k.isForeignKey && k.isPrimaryKey && k.source_column !== fkFieldName,
      );
      if (otherKeys.length === 1) {
        const otherKey = otherKeys[0];
        const otherModel = recase(
          this.caseModel,
        ...
        this.relations.push({
          ...
          childTable: qNameJoin(
            otherKey.foreignSources.target_schema || schema,
            otherKey.foreignSources.target_table,
          ),
          childId: otherId,
          joinModel: modelName,
          isOne: isOne,
  ...
  private getAlias(
    ...
    const singleName = singularize(name);
    if (isM2M) {
