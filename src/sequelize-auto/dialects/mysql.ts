...
export const mysqlOptions: DialectOptions = {
  ...
  isUnique: (record: FKRow, records: FKRow[]) => {
    ...
    return records.some(
      (row) =>
        row.constraint_name === record.constraint_name &&
        row.column_key.toUpperCase() === 'UNI',
    );
  },
  ...
  isPrimaryKey: (record: FKRow) => {
    return (
      _.isObject(record) &&
      _.has(record, 'constraint_name') &&
      record.constraint_name === 'PRIMARY'
    );
  },
